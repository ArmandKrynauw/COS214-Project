...
void WarEngine::loadCountries(const json& countries) {
    for (json c : countries) {
        ...
        factions.push_back(country);
    }

    // =========== Print Unit Names ===========
    // std::unordered_map<std::string, std::unordered_map<std::string, std::string> >::iterator it;
    // for (it = countryUnitNames.begin(); it != countryUnitNames.end(); ++it) {
