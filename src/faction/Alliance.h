...
#include "Faction.h"
/**
 * This class is the Composite of the Faction Composite Design Pattern
 * Hierarchy. The alliance may store many countries and alliances may join a
 * greater alliance.
 */
class Alliance : public Faction {
   ...
   public:
    ...
    /**
     * Provides functionality to generate resources for each country in the
     * alliance
     *
     * @param theatreResource The amount of resources from theatres to divide
     * among countries
     */
     ...
     * category is decreased by 500 points and the alliance is given the
     * appropriate advantage which is then provided to all the countries in the
     * alliance.
     *
     ...
     *
     * @return vector<Faction*> Vector of pointers to the faction in the
     * alliance
     */
    std::vector<Faction*> getMembers();
    /**
     * Provides functionality to get the alliance the the current alliance is in
     * if it is in a greater alliance.
     *
     * @return Alliance* Return if alliance is in a greater alliance
     * @return NULL Return if alliance is highest alliance and has no greater
     * alliance
     */
    virtual Alliance* getAlliance() override;
    /**
     * Provides functionality to command each faction in the alliance to clear
     * their casualties by calling clearCasualties()
     */
    virtual void clearCasualties() override;
    /**
     * Provides functionality to convert an alliance to JSON format. The object
     * will have the alliance name and an array of the countries that have
     * joined the alliance. (e.g. {"name": "Allies", "countries": ["America",
     * "France"]})
     *
