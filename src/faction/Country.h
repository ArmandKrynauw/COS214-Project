...
/**
 * This class is the Leaf of the Faction Composite Design Pattern Hierarchy. The
 * country stores entities, has an economy and may be join/leave and alliance.
 */
class Country : public Faction {
   ...
   public:
     ...
     * by removing the country from the alliance and then setting the alliance
     * pointer to NULL.
     */
    void leaveAlliance();
    /**
    ...
    /**
     * Provides functionality to change the mobilization of a country by
     * checking the escalation of the war and using the Mobilization state
     * pattern to decide if the mobilization of the country can change.
     *
     ...
     * current mobilization and morale of a Faction. The total resources from
     * theatres is then added.
     *
    ...
    /**
     * Provides functionality to invest research points into research for the
     * country If the country is in an alliance the the resources will be
     * invested into the alliances research vector.
     *
     * If the category that is invested in meets the goal of >= 500 points, the
     * category is decreased by 500 points and the country is given the
    ...
    /**
     * Provides functionality to return entities across all the country's
     * armedForces per theatre in JSON format.
     *
     * @return json JSON array of theatres containing JSON arrays of entities as
     * JSON objects
     */
    json getListOfUnits();
    /**
    ...
    int getEntityCount();
    /**
     * Provides functionality to traverse all armedForces owned by a country and
     * get the JSON format of each entity in said armedForces.
     *
     * @return json JSON array of entity JSON objects
     */
    json allUnitsToJSON();
    /**
     * Provides functionality to check if a country has units in any of the
     * zones
     *
    ...
    /**
     * Provides functionality to return the current research of a country in
     * JSON format. (e.g. {"name": "Germany", "industryCurrent":
     * 0,"propagandaCurrent": 0, "researchGoal": 500})
     *
